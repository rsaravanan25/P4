...
public class  PrefixComparator implements Comparator<Term> {
    ...
    public int compare(Term v, Term w) {
        int index = 0;
        int max = myPrefixSize;
        String vWord = v.getWord();
        String wWord = w.getWord();
        if(max > vWord.length()) max = vWord.length();
        if(max > wWord.length()) max = wWord.length();

        while(index < max)
        {
            int vNum = vWord.charAt(index);
            int wNum = wWord.charAt(index);
            if(vNum != wNum) return vNum - wNum;
            index++;
        }
        if (vWord.length() >= myPrefixSize && wWord.length() >= myPrefixSize) return 0;
