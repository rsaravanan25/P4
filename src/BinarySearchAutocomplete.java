...
public class BinarySearchAutocomplete implements Autocompletor {
	...
	public List<Term> topMatches(String prefix, int k) {
		...
		if (first <0 || last < 0 || k == 0) return new ArrayList<>();
		...
		while(first <= last)
		{
			Term trm = myTerms[first];
			if(!trm.getWord().startsWith(prefix)) continue;

			if(pq.size() < k) pq.add(trm);
			else if(pq.peek().getWeight() < trm.getWeight()) {
				pq.remove();
				pq.add(trm);
			}
			first++;
